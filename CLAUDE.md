...
- **Ngoại tuyến:** check-in mới và lộ trình lưu lúc mất mạng vào hàng chờ IndexedDB (`src/lib/outbox.js`, giữ được File ảnh), gửi lại khi mở app và sự kiện `online`. id tạo ở máy nên `createPlace`/`createTrack`/`uploadPhotos` phải idempotent (kiểm tra đã có, tên ảnh theo id, bỏ qua lỗi Storage 409). `Workspace` gộp mục chờ vào `places`/`tracks` với cờ `pending` (`kind: 'create' | 'edit'`). Sửa check-in lúc mất mạng là mục `edit` (id = id địa điểm; `enqueueEdit` gộp nhiều lần sửa, đè lên địa điểm đã lưu bằng `applyEdit`); sửa tiếp một nơi đang có bản sửa chờ thì luôn đi qua hàng chờ rồi gửi ngay, để bản cũ không đè bản mới. Hàng chờ chỉ xoá khi bấm Đăng xuất (có hỏi), không xoá theo sự kiện `SIGNED_OUT`.
- **Kế hoạch chuyến:** nơi `wishlist` có `trip_id` = điểm dừng của chuyến (dùng lại cột và RLS chuyến nhóm, không có bảng riêng). Thứ tự không lưu: `planOrder` (`src/lib/plan.js`) tính đường ngắn mỗi lần; bản đồ vẽ qua prop `plan` của `MapView`. `CheckinForm` giữ nguyên `trip_id` của nơi muốn đến. "Đã đến" = `api.markVisited` (đổi `kind`, giữ `trip_id`).
- **Chi phí chuyến:** bảng `trip_expenses` (VND số nguyên, `split_among uuid[]`), RLS theo `private.in_trip`; sửa, xoá: người tạo hoặc chủ chuyến. Realtime: `api.subscribeExpenses` (DELETE không lọc được theo `trip_id`, nhận là tải lại). Chia tiền, ai trả ai tính ở client (`src/lib/expenses.js`).
- **Nhập ảnh hàng loạt:** `src/lib/photoImport.js` gom ảnh (cùng ngày, ≤300 m) và dò nơi trùng (cùng ngày, ≤200 m → `api.addPhotos` vào nơi cũ); `PhotoImport` lấy tên bằng Nominatim tuần tự. `uploadPhotos` tự chuyển HEIC.
- **Gần nơi muốn đến, dự báo, sổ tay:** `App` lấy vị trí lúc mở app chỉ khi quyền định vị đã `granted` (không bật hộp hỏi), nhắc nơi muốn đến ≤500 m. Dự báo chuyến: `fetchForecast` (Open-Meteo chỉ tới hôm nay + 15 ngày). Sổ tay `TripBook` render qua portal ra ngoài `#root`; `@media print` ẩn `#root`.
- **Bảng xếp hạng, nhắc đi bộ:** cột `profiles.leaderboard` (tự chọn tham gia), `walk_reminder`, `goal_km`. RPC `friend_leaderboard` (chỉ số liệu tổng, rỗng nếu mình chưa tham gia), `walk_reminders` (chỉ service role). Edge Function `walk-reminder` và `memories` dùng chung `supabase/functions/_shared/push.ts`. Chuỗi ngày tính ở client (`src/lib/streak.js`).
