...
export default function Timeline({ places, tracks, onSelect, query, onQueryChange: setQuery }) {
  ...
  const visitedCount = useMemo(() => places.filter((p) => p.kind === 'visited').length, [places]);
  const walked = formatDistance(tracks.reduce((sum, t) => sum + (t.distance_m || 0), 0)).split(' ');
    ...
    <div className="screen">
      ...
      <div className="screen-inner">
        ...
        <dl className="stat-row">
          <div><dt>NƠI ĐÃ ĐẾN</dt><dd>{visitedCount}</dd></div>
          <div><dt>ĐÃ ĐI BỘ</dt><dd>{walked[0]}<small> {walked[1]}</small></dd></div>
          {/* Chờ dữ liệu ranh giới tỉnh (PLAN #21, #22) */}
          <div><dt>TỈNH, THÀNH</dt><dd>—<small> /34</small></dd></div>
        </dl>
        ...
        {showSearch && (
          <div className="journal-search">
            ...
            <div className="date-range">
              <label><span>Từ</span><input type="date" value={from} onChange={(e) => setFrom(e.target.value)} /></label>
        ...
        {places.length === 0 && (
          <p className="empty">Chưa có địa điểm nào. Mở Bản đồ, bấm Check-in rồi chạm lên bản đồ để ghim nơi đầu tiên.</p>
        )}
        {places.length > 0 && filtered.length === 0 && <p className="empty">Không có địa điểm khớp bộ lọc.</p>}

        {groups.map(([key, items]) => (
          <section key={key} className="month">
            <h2 className="month-title">
              {key === 'none' ? 'Chưa đặt ngày' : `Tháng ${Number(key.slice(5))}`}
              {key !== 'none' && <span className="mono-label">{key.slice(0, 4)}</span>}
            </h2>
...
function TimelineList({ items, urls, onSelect }) {
  ...
  items.forEach((p, i) => {
    ...
    dots.push({ x: textOnRight ? 72 : 28, y: top + (p.kind === 'wishlist' ? 14 : 44), kind: p.kind });
    top += ROW_H[p.kind];
  });
  ...
  return (
    <div className="timeline" style={{ height: top }}>
      <svg className="timeline-curve" viewBox={`0 0 100 ${top}`} preserveAspectRatio="none" aria-hidden="true">
        <path d={d} vectorEffect="non-scaling-stroke" />
      </svg>
      {dots.map((pt, i) => (
        <span key={i} className={`timeline-dot dot-${pt.kind}`} style={{ left: `${pt.x}%`, top: pt.y }} aria-hidden="true" />
      ))}
            ...
            <button className={`tl-entry${i % 2 ? ' is-flipped' : ''} tl-${p.kind}`} onClick={() => onSelect(p.id)}>
              ...
              <span className="tl-text">
                <span className="mono-label">
                  {p.kind === 'wishlist' ? 'MUỐN ĐẾN' : (p.visited_at ?? '').slice(5).split('-').reverse().join('/')}
                </span>
                <span className="tl-name">{p.name}</span>
                {p.kind === 'visited' && <span className="muted-sm">{placeSummary(p)}</span>}
              </span>
            </button>
          </li>
        ))}
