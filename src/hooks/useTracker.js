...
function saveRecovery(startedAt, points) {
  try {
    localStorage.setItem(RECOVERY_KEY, JSON.stringify({ startedAt, points }));
  } catch {
...
export function useTracker({ maxAccuracy = 30, maxSpeedKmh = 15, minStepM = 3 } = {}) {
  ...
  const handlePosition = useCallback(
    (pos) => {
      ...
      if (prev) {
        ...
        const tooFast = dtSec > 0 && (d / dtSec) * 3.6 > maxSpeedKmh;
      ...
      setPoints(next);
      saveRecovery(startedAtRef.current, next);
    },
  ...
  // Dừng ghi, giữ lại điểm để người dùng lưu. Trả về danh sách điểm.
  ...
  return { recording, points, startedAt, accuracy, error, screenLocked, start, stop, reset };
}
