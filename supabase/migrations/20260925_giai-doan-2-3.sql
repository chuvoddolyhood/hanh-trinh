-- =====================================================================
-- Migration: chuyến đi, link chia sẻ, vùng riêng tư → giai đoạn 2 (bạn bè, bình luận,
-- chuyến nhóm) → giai đoạn 3. Dùng cho database ĐÃ chạy schema.sql bản đầu tiên
-- (chỉ có places, photos, tracks và bucket ảnh). Dán toàn bộ vào Supabase SQL Editor, bấm Run một lần.
--
-- - Không xoá, không sửa dữ liệu có sẵn: chỉ thêm bảng, cột rỗng (trip_id), policy, hàm, trigger;
--   tạo hồ sơ + link mời cho tài khoản đã có.
-- - Chạy trong một transaction: lỗi ở bất kỳ đâu thì không thay đổi nào được áp dụng.
-- - Chạy lại nhiều lần vẫn được (if not exists, drop ... if exists, on conflict).
-- - Nội dung giống supabase/schema.sql từ mục "Chuyến đi, link chia sẻ, vùng riêng tư" tới cuối;
--   database mới thì chạy schema.sql.
-- =====================================================================
...
-- ---------------------------------------------------------------------
-- Kiểm tra sau khi chạy (chạy riêng câu này):
-- ---------------------------------------------------------------------
-- select
--   (select count(*) from auth.users)            as so_tai_khoan,
--   (select count(*) from public.profiles)       as so_ho_so,       -- = số tài khoản
--   (select count(*) from public.invites)        as so_link_moi,    -- = số hồ sơ
--   to_regclass('public.trip_members')           as bang_thanh_vien,
--   to_regclass('public.push_subscriptions')     as bang_push,
--   (select count(*) from pg_publication_tables
--     where pubname = 'supabase_realtime'
--       and tablename in ('comments', 'reactions')) as realtime;     -- = 2
